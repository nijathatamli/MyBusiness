...
# ─────────────────────────────────────────
# FRONTEND ROUTES (serve existing HTML/CSS/JS)
# ─────────────────────────────────────────
...
def frontend_dashboard():
    # Current frontend is a single-page flow in index.html.
    return send_from_directory(".", "index.html")
...
def init_database() -> None:
    """Create tables and provide actionable setup hints if PostgreSQL is unavailable."""
    with app.app_context():
        try:
            db.create_all()
        except OperationalError as exc:
            db_url = app.config["SQLALCHEMY_DATABASE_URI"]
            print("\n[MyBusiness] Database connection failed.")
            print(f"Configured DATABASE_URL: {db_url}")
            print("\nFix it with one of these options:")
            print("1) Start PostgreSQL locally and create database 'mybusiness'.")
            print("2) Or run PostgreSQL with Docker:")
            print("   docker run --name mybusiness-postgres -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=mybusiness -p 5432:5432 -d postgres:16")
            print("3) Then export DATABASE_URL, for example:")
            print("   export DATABASE_URL='postgresql+psycopg2://postgres:postgres@localhost:5432/mybusiness'")
            print("\nOriginal error:")
            print(exc)
